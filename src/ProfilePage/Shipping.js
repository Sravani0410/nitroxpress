...
const Shipping = () => {
  ...
  const [pickuppincodeactivecheck, setPickupPinCodeActiveCheck] =
    useState(false);
  const [deliverypincodeactivecheck, setDeliveryPinCodeActiveCheck] =
    useState(false);
  ...
  const [yesnoactivebuttonInsurance, setYesNoActiveButtonInsurance] =
    useState(false);
  ...
  const GetShipmentDetailsData = useSelector(
    (state) => state.GetShipmentDetailsReducer.GetShipmentDetailsData.data
  );
  const PostPincodesAvailabilityReducer = useSelector(
    (state) =>
      state.PostPincodesAvailabilityReducer.PostPincodesAvailabilityData?.data
  );
  const PostPincodesDeliveredReducer = useSelector(
    (state) =>
      state.PostPincodesDeliveredReducer.PostPincodesDeliveredData?.data
        ?.message
  );
  const PostShipmentDetailsData = useSelector(
    (state) =>
      state.PostShipmentDetailsReducer.PostShipmentDetailsData.data?.shipment_id
  );
  const PostDeliveryAddressData = useSelector(
    (state) =>
      state.PostDeliveryAddressReducer.PostDeliveryAddressData.data
        ?.delivered_id
  );
  const PostPickupAddressData = useSelector(
    (state) =>
      state.PostPickupAddressReducer.PostPickupAddressData?.data?.pickup_id
  );
  const PatchPickupAddressData = useSelector(
    (state) => state.PatchPickupAddressReducer.PatchPickupAddressData?.data
  );
  const OrderDetails = useSelector(
    (state) => state.orderDetailsReducer.orderDetails
  );
  const ShipmentLoaderTrueFalseData = useSelector(
    (state) => state.ShipmentLoaderTrueFalseReducer.ShipmentLoaderTrueFalseData
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      let payload = {
        pickup_id: PostPickupAddressData
          ? PostPickupAddressData
          : pickuppatchobjectid,
  ...
  const ConfirmDeleverFun = (e) => {
    ...
    let Eway_bill_id = sessionStorage.getItem("Eway_bill_id", false);
    ...
    let payloadShipping = {
      product_type:
        otherProductType && selectedproduct == "OTHERS"
          ? otherProductType
          : selectedproduct,
    ...
    priceValue == "none"
      ...
      : priceValue && priceValue >= 50000 && ewayPdf == "none"
      ? toast.warn("Please upload eway bill")
      : ewayPdf && pickupname && pickupcontact.length !== 10
      ? toast.warn("pickup contact number is invalid")
      : pickupcontact &&
        pickupaddress &&
        pickuppincode &&
        pickupcity &&
      ...
      ? toast.warn("delivery contact number is invalid")
      : deliverycontact &&
        deliveryaddress &&
        deliveredpincode &&
        pickupaddressactive &&
        deliveryaddressactive && //this is new modification and address's is not validated
        ...
        deliverystate &&
        selectedproduct &&
        deliveryproduct == "none"
      ? toast.warn("Please select delivery type")
      : deliveryproduct && weight && date
      ? dispatch(
          PostShipmentDetails(
            payloadShipping,
  ...
  useEffect(() => {
    if (
      PostPincodesAvailabilityReducer?.type == "pickup" &&
      PostPincodesAvailabilityReducer?.message === "Available"
    ) {
      ...
      PostPincodesAvailabilityReducer?.type === "delivered" &&
      PostPincodesAvailabilityReducer?.message === "Available"
    ) {
  ...
  useEffect(() => {
    ...
    if (PostPincodesAvailabilityReducer?.type === "pickup") {
      if (PostPincodesAvailabilityReducer?.message === "Available") {
        ...
        setDeliveryTypeError(false);
        if (
          pickuppatchfilterdata &&
          pickuppatchfilterdata?.length !== 0
          // && !pickuppincodeactive
        ) {
      ...
      if (PostPincodesAvailabilityReducer?.message === "Available") {
        ...
        setDeliveryTypeError(false);
        if (
          deliveryPatchFilterData &&
          deliveryPatchFilterData?.length !== 0
          // && !pickuppincodeactive
        ) {
          if (deliveryproduct == null || deliveryproduct == "none") {
            setPickUpPopup(true);
  ...
  useEffect(() => {
    if (
      PostPincodesAvailabilityReducer == undefined ||
      PostPincodesAvailabilityReducer.message == "Pin code is not available"
    ) {
      // setDeliveryTypeError(true)
      // setDeliveryProduct("")
      // setPickUpPopup(false)
  ...
  const PaymentFun = async (e, amount) => {
    ...
    let bodyContent = JSON.stringify({
      ...
      delivered_address_id:
        GetShipmentDetailsData?.user_delivered_location[0]?.id,
      ...
      amount_format: {
        ...
        cod_percent_price:
          GetShipmentDetailsData?.calculation_details?.amount_format
            ?.cod_percent_price,
        gst: GetShipmentDetailsData?.calculation_details?.amount_format?.gst,
        insurance:
          GetShipmentDetailsData?.calculation_details?.amount_format?.insurance,
        packaging_price:
          GetShipmentDetailsData?.calculation_details?.amount_format
            ?.packaging_price,
        insurance_price:
  ...
  let array = [
    ...
    { value: "EXPRESS_DELIVERY", key: "Express delivery" },
    { value: "AIR_DELIVERY", key: "Air delivery" },
    { value: "SURFACE_DELIVERY", key: "Surface Delivery" },
    ...
    {
      value: "REVERSE_PICKUP_SERVICE_WITH_SAME_PRICE",
      key: "Reverse pickup service with same price",
    },
    {
      value: "INTERNATIONAL_COURIER_SERVICE",
      key: "Internation Courier service",
    },
          ...
          <div className="row">
            <div className="col-md-6">
              <div className="shippingleft-part">
                ...
                <form className="row">
                  <div className="form-box col-12 mb-3">
                    ...
                    <input
                      ...
                      className={`form-control check-box  ${
                        !pickuppincodeactive && pickuppincode
                          ? "alert_border"
                          : ""
                      } `}
                      placeholder="Pickup Pincode"
                      value={pickuppincode}
                      onChange={(e) => PickupPinCodeFun(e)}
                    />
                    {!pickuppincodeactive && pickuppincode && (
                    ...
                    <span className="check-img ">
                      ...
                      >
                        ...
                        <path
                          d="M0.00012207 0H35.0001C37.2093 0 39.0001 1.79086 39.0001 4V37C39.0001 39.2091 37.2093 41 35.0001 41H0.00012207V0Z"
                          fill="#FFC900"
                          fillOpacity={pickuppincodeactive ? "1" : "0.1"}
                        />
                        <path
                          d="M0.00012207 -1H35.0001C37.7615 -1 40.0001 1.23858 40.0001 4H38.0001C38.0001 2.34315 36.657 1 35.0001 1H0.00012207V-1ZM40.0001 37C40.0001 39.7614 37.7615 42 35.0001 42H0.00012207V40H35.0001C36.657 40 38.0001 38.6569 38.0001 37H40.0001ZM0.00012207 41V0V41ZM35.0001 -1C37.7615 -1 40.0001 1.23858 40.0001 4V37C40.0001 39.7614 37.7615 42 35.0001 42V40C36.657 40 38.0001 38.6569 38.0001 37V4C38.0001 2.34315 36.657 1 35.0001 1V-1Z"
                          fill="#EDEDED"
                          mask="url(#path-1-inside-1_347_16599)"
                        />
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M27.4295 14.0128C27.2657 14.0367 27.146 14.0743 26.9674 14.158C26.8055 14.2339 26.59 14.429 25.9048 15.1196C25.4752 15.5526 24.9631 16.0647 24.3148 16.7093C24.0919 16.931 23.7333 17.2887 23.5179 17.5041C23.1675 17.8547 21.1614 19.8593 20.2325 20.7871C20.0358 20.9835 19.6782 21.3412 19.4378 21.5819C19.1974 21.8227 18.8954 22.1236 18.7668 22.2507C18.6382 22.3777 18.4414 22.5727 18.3294 22.6839C18.2174 22.7952 18.1164 22.883 18.1049 22.8791C18.0844 22.8721 17.9665 22.7733 16.4923 21.5268C16.0818 21.1797 15.5479 20.7283 15.3059 20.5236C14.2217 19.6069 14.174 19.569 14.0043 19.4884C13.8317 19.4065 13.5352 19.3353 13.3694 19.336C13.2036 19.3368 12.9384 19.4023 12.7511 19.4889C12.2536 19.7189 11.9335 20.2809 12.0117 20.7869C12.0853 21.2627 12.1646 21.3627 13.0826 22.1374C13.1894 22.2275 13.6123 22.5842 14.0224 22.93C14.4325 23.2759 15.1743 23.902 15.671 24.3213C17.4601 25.832 17.3978 25.7816 17.5795 25.866C17.7981 25.9674 17.9441 25.9988 18.2029 26C18.5088 26.0014 18.8108 25.9057 19.0418 25.7342C19.0931 25.6962 19.5105 25.2906 19.9693 24.833C20.4282 24.3754 20.9753 23.8299 21.1851 23.6208C21.395 23.4117 21.8577 22.9501 22.2134 22.595C22.5692 22.2399 23.213 21.5971 23.6442 21.1666C24.0755 20.7361 24.9934 19.8189 25.6841 19.1283C26.3747 18.4377 27.035 17.7794 27.1514 17.6654C27.648 17.1786 28.5891 16.2332 28.6838 16.1259C29.1387 15.6106 29.0986 14.8604 28.5905 14.379C28.2969 14.1009 27.8316 13.9542 27.4295 14.0128Z"
                          fill="white"
                        />
                      </svg>
                    </span>
                  ...
                  <div className="form-box col-12 ">
                    ...
                    <input
                      ...
                      maxlength={6}
                      onWheel={handleOnWheel}
                      ref={deliverPincodeRef}
                      onKeyDown={handleKeyDown}
                      className={`form-control check-box ${
                        !deliverypincodeactive && deliveredpincode
                          ? "alert_border"
                          : ""
                      }`}
                      placeholder="Delivered Pincode"
                      value={deliveredpincode}
                ...
                <div
                  className={`${
                    deliverypincodeactive && pickuppincodeactive
                      ? " "
                      : "disable_opercity"
                  } `}
                >
                  ...
                  <form className="row">
                    ...
                    <div className="form-box col-12 mb-3">
                      ...
                      {priceValue >= 50000 ? (
                        <div className="input_filed input_file  mt-3">
                    ...
                    <div className="form-box select-arrow col-md-6 mb-3">
                      ...
                      {deliverytypeerror && (
                        <span className="text-danger ">
                          <small> Delivery type is not available </small>
                        </span>
                      )}
                    </div>
                    ...
                    <div className="shipment-box pt-3">
                      ...
                      <div className="shipment-btn">
                        <button
                          type="button"
                          className={
                            yesnoactivebutton ? "active yes-btn" : "yes-btn"
                          }
                          onClick={(e) => setYesNoActiveButton(true)}
                        >
                          Yes
                        </button>
                        ...
                        <button
                          type="button"
                          className={
                            !yesnoactivebutton ? "active no-btn" : "no-btn"
                          }
                          onClick={(e) => setYesNoActiveButton(false)}
                        >
                          No
                        </button>
                      </div>
                    ...
                    {yesnoactivebutton && (
                      <div className="form-box select-arrow col-12 mb-3">
                        ...
                        >
                          <option value="DEFAULT" disable selected hidden>
                            Select Package Type
                          </option>
                    ...
                    <div className="shipment-box pt-3">
                      ...
                      <div className="shipment-btn">
                        <button
                          type="button"
                          className={
                            yesnoactivebuttonInsurance
                              ? "active yes-btn"
                              : "yes-btn"
                          }
                          onClick={(e) => setYesNoActiveButtonInsurance(true)}
                        >
                          Yes
                        </button>
                        ...
                        <button
                          ...
                          onClick={(e) => setYesNoActiveButtonInsurance(false)}
                        >
                          No
                        </button>
                      </div>
              ...
              <div className="shipmentright-part">
                ...
                <p>
                  {!GetShipmentDetailsData
                    ...
                    : GetShipmentDetailsData &&
                      `${GetShipmentDetailsData.user_pickup_location[0].address},
                ...
                <p>
                  {!GetShipmentDetailsData
                    ...
                    : GetShipmentDetailsData &&
                      `${GetShipmentDetailsData.user_delivered_location[0].address},
                  ...
                  <div className="col-md-8 col-6">
                    ...
                    <p className="mb-0">
                      {!GetShipmentDetailsData
                        ? "Select your date"
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.user_shipment_details[0].pickup_date}`}
                    </p>
                  </div>
                  <div className="col-md-4 col-6">
                    ...
                    <p className="mb-0">
                      {!GetShipmentDetailsData
                        ...
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.user_shipment_details[0].delivery_type}`}
                    </p>
                  </div>
                </div>
                <hr />
                ...
                <div className="row">
                  ...
                  <div className="col-md-4 col-6">
                    <h4>
                      ...
                      {!GetShipmentDetailsData
                        ? "0"
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.calculation_details.shipping_cost}`}
                      /-
                    </h4>
                  </div>
                  <div className="col-md-8 col-6">
                  ...
                  <div className="col-md-8 col-6">
                    <p className="mb-2">Packaging Cost</p>
                  </div>
                  <div className="col-md-4 col-6">
                    <h4>
                      Rs.{" "}
                      {!GetShipmentDetailsData
                        ? "0"
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.calculation_details.pacakaging_cost}`}
                      /-
                    </h4>
                  </div>
                  <div className="col-md-8 col-6">
                    <p className="mb-2">Insurance Cost</p>
                  </div>
                  <div className="col-md-4 col-6">
                    <h4>
                      Rs.{" "}
                      {!GetShipmentDetailsData
                        ? "0"
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.calculation_details.insurance_price}`}
                      /-
                    </h4>
                  </div>
                </div>
                  ...
                  <div className="col-md-4 col-6">
                    <h2>
                      ...
                      {!GetShipmentDetailsData
                        ? "0"
                        : GetShipmentDetailsData &&
                          `${GetShipmentDetailsData.calculation_details.total_price}`}
                      /-
                    </h2>
                  </div>
                </div>
                <button
                  type="button"
                  className={`${
                    GetShipmentDetailsData ? "btn" : "disable_opercity btn"
                  } `}
                  onClick={(e) =>
                    GetShipmentDetailsData &&
                    PaymentFun(
            ...
            <div className="justify-content-center  bg-white">
              <div className="d-flex justify-content-between px-3 pt-3">
                ...
                <h4
                  className="pe-3"
                  role="button"
                  onClick={(e) => setPickUpPopup(false)}
                >
                  {" "}
                  X{" "}
                </h4>
                  ...
                  OrderDetails?.data?.map((item, id) => {
                    return (
                      <div className="d-flex justify-content-between">
                        ...
                        <div
                          className="text-warning pe-3 pt-4  "
