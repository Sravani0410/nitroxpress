...
import {
  GetAdminCloneOrder,
  GetAdminOrderGenerateOrderId,
  GetAdminOrderPaymentOrder,
...
const User = () => {
  ...
  const GetAdminOrderGenerateOrderIdData = useSelector(
    (state) =>
      state.GetAdminOrderGenerateOrderIdReducer.GetAdminOrderGenerateOrderIdData
        ?.data
  );
  const PostPincodesDeliveredReducer = useSelector(
    (state) => state.PostPincodesDeliveredReducer.PostPincodesDeliveredData
  );
  const GetAdminOrderPaymentOrderData = useSelector(
    (state) =>
      state.GetAdminOrderPaymentOrderReducer.GetAdminOrderPaymentOrderData
  );
  const GetAdminCloneOrderData = useSelector(
    (state) => state.GetAdminCloneOrderReducer.GetAdminCloneOrderData?.data
  );
  ...
  useEffect(() => {
      ...
      if (PostPincodesAvailabilityReducer?.type == "pickup") {
        ...
        if (
          ...
          sessionStorage.setItem(
            "ZoneData",
            PostPincodesAvailabilityReducer.zone
          );
    ...
    GetAdminOrderGenerateOrderIdData &&
      sessionStorage.setItem(
        "product_order_id",
        GetAdminOrderGenerateOrderIdData?.product_order_id
      );
  ...
  const DeliverhandleSelect = async (address) => {
    setDeliveryAddress(address);
    if (address == undefined) {
      setDeliveryAddress(" ");
      setDeliverAddressActive(true);
  ...
  const BasicInformationNextBtn = async () => {
    sessionStorage.setItem("PaymentMethod", ordertype);
    sessionStorage.setItem("ConsignerPinCode", pickuppincode);
    ...
    CompanyName == null
      ...
      ? toast.warn(" Please fill pickup name ")
      : pickupname && pickupnumber?.length !== 10
      ...
      : pickupnumber &&
        ...
        pickupcountrycode &&
        delivername === ""
      ? toast.warn(" Please fill deliver name ")
      : delivername && delivernumber?.length !== 10
      ? toast.warn("deliver contact number is invalid")
      : delivernumber &&
        deliveraddress &&
        deliverpincode &&
        ...
        ordertype == null
      ? toast.warn("Please Select Correct Order type")
      : ordertype
  ...
  let UserDetailOfLocalStorage = JSON.parse(
    sessionStorage.getItem("UserDetailsPayload", false)
  );

  useEffect(() => {
    let splitPickupCountryCode =
      UserDetailOfLocalStorage?.pickup_phone_number?.split("-");
    let splitDeliveredCountryCode =
      UserDetailOfLocalStorage?.delivered_phone_number?.split("-");
    if (UserDetailOfLocalStorage !== null) {
              ...
              <datalist id="brow">
                {pagepathdata &&
                  pagepathdata?.map((item, id) => {
                    ...
                    <div
                      className={`${
                        CompanyName != null ? "" : "disable_opercityOrder1"
                      } row pe-0`}
                    >
                    ...
                    /> */}
                      ...
                      <div className="col-md-4 mt-2">
                        ...
                        <input
                          ...
                          onKeyDown={handleKeyDown}
                          className={`form-control check-box ${
                            deliverpincodeactive && deliverpincode
                              ? "alert_border"
                              : ""
                          }`}
                          placeholder="Delivered Pincode"
                          value={deliverpincode}
                          onChange={(e) => DeliverpincodeFun(e)}
                        />
                        {deliverpincodeactive && deliverpincode ? (
                          <span className="text-danger ">
                            <small> Pincode is not available </small>
                      ...
                      <div className="col-12 mt-3">
                        ...
                        <PlacesAutocomplete
                          ...
                          // onSelect={(e) => DeliverhandleSelect(e)}
                        >
                          {({
                            getInputProps,
                            suggestions,
                            ...
                            <div className={deliveraddressactive ? "mb-4" : ""}>
                              <input
                                {...getInputProps({
                                  placeholder: "Search Places",
                                  className: "location-search-input",
                    ...
                    <button type="button" className="btn gen-btn"
                      ...
                      <option value="COD">cod</option>
                        ...
                        <select
                          ...
                          // onBlur={(event) => event.target.size = 0}
                        ...
                        >
                          <option
                            value="none"
                            selected={ordertype == undefined}
                          >
                            Select Order Type
                          </option>
                          {orderType?.map((item, id) => {
                            return (
                              <option value={item?.value}>{item?.data}</option>
                            );
                          })}
                        </select>
                        {PostPincodesAvailabilityReducer &&
