...
const Orderinner = () => {
  ...
  const [TextRemarkFieldData, setTextRemarkFieldData] = useState("");
  ...
  const GetGoogleCityStateDataReducer = useSelector(
    (state) => state.GetGoogleCityStateReducer?.GetGoogleCityStateData
  );
  ...
  useEffect(() => {
    ...
    setDeliveryAddress(
      GetAdminOrderReturnData?.data?.customer_details?.address
    );
    setCompanyName(
      GetAdminOrderReturnData?.data?.customer_details?.company_name
    );
    ...
    setHeight(
      GetAdminOrderReturnData?.data?.Item_summary?.height == null
        ? 0
        : GetAdminOrderReturnData?.data?.Item_summary?.height
    );
    setWidth(
      GetAdminOrderReturnData?.data?.Item_summary?.breadth == null
        ? 0
        : GetAdminOrderReturnData?.data?.Item_summary?.breadth
    );
    setLength(
      GetAdminOrderReturnData?.data?.Item_summary?.length == null
        ? 0
        : GetAdminOrderReturnData?.data?.Item_summary?.length
    );

    setLandMark(GetAdminOrderReturnData?.data?.delivered_address?.landmark);
    setCountryPinCode(
      GetAdminOrderReturnData?.data?.delivered_address?.country_code
    );
    setPhone(GetAdminOrderReturnData?.data?.delivered_address?.phone);
    setQuantity(
      GetAdminOrderReturnData?.data?.Item_summary?.quantity == null
        ? 0
        : GetAdminOrderReturnData?.data?.Item_summary?.quantity
    );
    ...
    setYesNoActiveButtonInsurance(
      Boolean(GetAdminOrderReturnData?.data?.Item_summary?.insurance)
    );
    setPackShipment(GetAdminOrderReturnData?.data?.Item_summary?.pack_shipment);
    setAmount(
      Number(GetAdminOrderReturnData?.data?.order_summary?.total_amount)
    );
    setBasePrice(
      Number(GetAdminOrderReturnData?.data?.order_summary?.base_price)
    );
  ...
  const UpdateFun = (e) => {
    ...
    if (data == "online") {
      ...
      let payloadData = {
        ...
        zone: zone,
        total_amount: PostAdminOrderPaymentCalReducerData?.data?.total_price
    ...
    if (editslidebar == true) {
      ...
      let PayloadData = {
        ...
        packaging:
          packShipmentPrice != ""
            ? "OTHER"
            : GetAdminOrderReturnData?.data?.Item_summary?.packaging,
        pack_shipment:
          packShipmentPrice != ""
            ? true
            : GetAdminOrderReturnData?.data?.Item_summary?.pack_shipment,
        ...
        zone: zone,
        method:
          GetAdminOrderReturnData?.data?.user_type == "B2B"
            ? GetAdminOrderReturnData?.data?.method
            : "",
      ...
      if (GetAdminOrderReturnData?.data?.delivered_address?.zone != "OTHER") {
        ...
        GetAdminOrderReturnData?.data?.delivered_address?.zone == "OTHER"
      ) {
        dispatch(PostAdminOrderPaymentCal(PayloadData));
      }
    }
  ...
  useEffect(() => {
    if (GetAdminOrderReturnData?.data?.delivery_partner === "SKYKING") {
      ...
      GetAdminOrderReturnData?.data?.delivery_partner === "XPRESSBEES"
    ) {
      setDeliveryLogo("/images/XPRESS.png");
    } else if (
      GetAdminOrderReturnData?.data?.delivery_partner === "DELHIVERY"
    ) {
  ...
  useEffect(() => {
    if (
      PostOrderDownloadInvoiceFileData.status == 200 &&
      downloadInvoiceTrue == true
    ) {
  ...
  const date = new Date(
    GetAdminOrderReturnData?.data?.order_summary?.order_created
  );
  ...
  useEffect(() => {
    ...
    if (PostPincodesAvailabilityReducer) {
      ...
      if (PostPincodesAvailabilityReducer?.type == "pickup") {
        if (
          PostPincodesAvailabilityReducer?.message !==
          "Pin code is not available"
        ) {
  ...
  const CloseRemarkfun=()=>{
    setViewRemark(false)
    // if(location?.state?.openpopup==true){
    //   setViewRemark(false)
    // }
    // else if(location?.state?.closepopup==false){
    //   setViewRemark(false)
    // }
    // else{
    //   setViewRemark(false)
  ...
  //   }
  //   // setViewRemark(false)
  // },[location?.state?.openpopup,location?.state?.closepopup])

  const AddRemarkConfirmFun = () => {
    let payload = {
      order_id: param.id,
      remark: TextRemarkFieldData,
    };
    dispatch(PostAddRemark(payload));
    setAddRemark(false);
    setTextRemarkFieldData("");
  };
  const TextRemarkFun = (e) => {
    setTextRemarkFieldData(e.target.value);
  };
  return (
    <>
      <div className={`${ToggleFunData ? "collapsemenu" : ""}`}>
            ...
            {paramHash?.hash == "#booked" && editslidebar && (
              <div className="editpopup-outer">
                <div className="editpopup popup-box">
                  <h2>
                    Edit Order Details ({OrderTypeData}) -{" "}
                    <span> {param.id}</span>{" "}
                  ...
                  <div className="editpopup-body">
                    ...
                    <div>
                      ...
                      <div className="row mt-2">
                        ...
                        {GetAdminOrderReturnData?.data?.delivered_address
                          ?.zone != "OTHER" && (
                          <div className="mt-3">
                            ...
                            <div className="pacform-box">
                              <input
                                ...
                                onChange={(e) =>
                                  setPackShipmentPrice(e.target.value)
                                }
                              />
                            </div>
                          </div>
                        ...
                        <div className="shipment-box pt-3">
                          ...
                          <div className="shipment-btn">
                            <button
                              ...
                              onClick={(e) =>
                                setYesNoActiveButtonInsurance(true)
                              }
                            >
                              Yes
                            </button>
                            <button
                              ...
                              onClick={(e) =>
                                setYesNoActiveButtonInsurance(false)
                              }
                            >
                              No
                            </button>
                    ...
                    <div className="row mt-3">
                      ...
                      <div className="mt-3">
                        <input
                          ...
                          // PostAdminOrderPaymentCalReducerData.data.total_price
                          onChange={(e) =>
                            zone == "OTHER" ? setBasePrice(e.target.value) : ""
                          }
                        />
                      </div>
                      <h3>Total Amount</h3>
            ...
            <div className="ordernumber-part">
              <div className="left-part">
                <div className="box summery-table">
                  <table>
                    <tr>
                      ...
                      <th>Qty</th>
                      <th>Weight</th>
                      {GetAdminOrderReturnData?.data?.method === "COD" ? (
                        <th>COD</th>
                      ) : (
                        <th>Prepaid</th>
                      )}
                    ...
                    <tr>
                      ...
                      <td>
                        {
                          GetAdminOrderReturnData?.data?.Item_summary
                            ?.product_type
                        }
                      </td>
                      <td>
                      ...
                      <td>
                        {GetAdminOrderReturnData?.data?.Item_summary?.weight}
                      </td>
                      <td>
                        {GetAdminOrderReturnData?.data?.method === "COD"
                          ? GetAdminOrderReturnData?.data?.Item_summary
                ...
                <div className=" box customer-part">
                  <div className="customer-details-part">
                    <h2>Customer Details</h2>
                    <div className="orderinfobtn-group">
                      <button
                        type="button"
                        // className="btn add-remark-btn"
                        className={`${ PermissionData()?.ALLOW_ADD_REMARK_ACTION ==
                          "ALLOW_ADD_REMARK_ACTION"
                            ?"btn add-remark-btn":"btn add-remark-btn permission_blur"}`}
                        onClick={(e) =>
                          PermissionData()?.ALLOW_ADD_REMARK_ACTION ==
                          "ALLOW_ADD_REMARK_ACTION"
                            ? AddRemarkfun(e)
                            : ""
                        }
                      ...
                      <button
                        ...
                        className={`${ PermissionData()?.ALLOW_VIEW_ORDER_REMARK_ACTION ==
                          "ALLOW_VIEW_ORDER_REMARK_ACTION"
                            ?"btn action-btn":"btn action-btn permission_blur"}`}
                        onClick={(e) =>  PermissionData()?.ALLOW_VIEW_ORDER_REMARK_ACTION ==
                          "ALLOW_VIEW_ORDER_REMARK_ACTION"
                            ? Viewremarkfun(e):""}
                  ...
                  <ul>
                    <li>
                      ...
                      <p>
                        {" "}
                        {
                          GetAdminOrderReturnData?.data?.customer_details?.name
                        }{" "}
                      </p>
                    </li>
                    <li>
                      ...
                      <p>
                        {
                          GetAdminOrderReturnData?.data?.customer_details
                            ?.address?.phone
                        }
                      </p>
                    </li>
                    <li>
                      ...
                      <p>
                        {" "}
                        {GetAdminOrderReturnData?.data?.customer_details?.email}
                      </p>
                    </li>
                    <li>
              ...
              <div className="right-part">
                <div
                  className={`box delivery-box mb-3 ${
                    ...
                    "VIEW_ORDER_TRACK_PAGE" ? (
                      <button
                        ...
                        className={`btn btn-ship  ${
                          PermissionData()?.VIEW_TRACK_SEARCH_DASHBOARD ==
                          "VIEW_TRACK_SEARCH_DASHBOARD"
                            ? " "
                            : "permission_blur"
                        }`}
                        onClick={(e) =>
                  ...
                  <ul>
                    ...
                    <li className="mt-4 pt-1">
                      ...
                      <p>
                        ...
                        {
                          GetAdminOrderReturnData?.data?.order_summary
                            ?.order_update_date
                        }
                      </p>
                    </li>
        ...
        {addremark && (
          <div className="popupouter remark-popup">
            <div className="popupinner">
              ...
              <div className="col-sm-12 pt-3">
                <label className="remarkname">Remarks</label>
                <p className="">Any Instructions</p>
                <textarea
                  className="w-100 p-2"
                  rows="5"
                  cols="70"
                  placeholder="Write Here...."
                  value={TextRemarkFieldData}
                  onChange={(e) => TextRemarkFun(e)}
                />
              ...
              <div className="remarkgroups ">
                <button
                  type="button"
                  className="save-btn"
                  onClick={(e) =>
                    TextRemarkFieldData?.length > 2
                      ? AddRemarkConfirmFun(e)
                      : ""
                  }
                >
                  Save
        ...
        {viewremark && (
          <div className="callbuyerpopup_outer ">
            <div className="callbuyerpopup">
              <h2>View Remark Details </h2>
              <div
                className="close-btn"
                type="button"
                onClick={(e) => CloseRemarkfun(e)}
              >
                <svg
                  width="73"
                  height="73"
              ...
              <div className="popup-body ">
                <div className="tab-content remark-box">
                  <table>
                    <tr>
                      <th>Created At</th>
                      <th>Created Time</th>
                    ...
                    {GetAdminOrderReturnData?.data?.remark_details?.map(
                      (items, id) => {
                        return (
                          <tr>
                            <td>
                            ...
                            <td>{new Date(items?.created_at).toLocaleTimeString()}</td>
                            <td className="remarkword">{items?.remark}</td>
                          </tr>
                        );
                      }
                    )}
